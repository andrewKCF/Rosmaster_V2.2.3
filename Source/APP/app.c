	...
	while (System_Enable())
	{
		if (Key1_State(KEY_MODE_ONE_TIME))
		{
			...
			if (key_state)
			{
				...
				// Mecanum_State(MOTION_RIGHT, 500, 0);
				// int speed = 3000;
				// Motion_Set_Pwm(speed, speed, speed, speed);
				// speed = -speed;
				// Motion_Set_Pwm(2000, 2000, 2000, 2000);
				// UartServo_Ctrl(0xfe, 1000, 500);
				// PwmServo_Set_Angle_All(30, 30, 30, 30);
			}
			else
			{
				key_state = 1;
				...
				Motion_Stop(1);
				// UartServo_Ctrl(0xfe, 3000, 500);
				// PwmServo_Set_Angle_All(130, 130, 130, 130);
